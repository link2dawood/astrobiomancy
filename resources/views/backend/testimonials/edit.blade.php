                ...
                <div class="row m-b-40">
                    <div class="col-md-12">
                        <div class="well white">
                            <fieldset>
                                <legend>Edit testimonial — {{ $item->name }} ({{ strtoupper($item->lang) }})</legend>
                                <form action="{{ url('dashboard/testimonials/update/' . $item->id) }}" method="POST" enctype="multipart/form-data">
                                    {{ Form::input('hidden', '_token', csrf_token()) }}
                                    @include('backend.testimonials._form', ['item' => $item])
