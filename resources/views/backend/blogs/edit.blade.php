  ...
  <main>
	...
	<div class="main-container">
       ...
       <div class="well white">
				...
				<form action="{{ url('dashboard/blog/post/update_action') }}" method="POST" enctype="multipart/form-data">
						...
						<div class="row">
							...
							<div class="form-group" >
								<label for="inputEmail" class="control-label">Title</label>
									{{ Form::input('text', 'title', $blog->title, ['class' => 'form-control', 'id' => '']) }}

									{{ Form::input('hidden', 'id', $blog->id, ['class' => 'form-control', 'id' => '']) }}
							</div>
							</div>
							<div class="col-md-4">
							<div class="form-group" >
								<label for="inputEmail" class="control-label">Meta Keyword</label>
									{{ Form::input('text', 'meta_keyword',$blog->meta_keyword, ['class' => 'form-control', 'id' => '']) }}
							</div>
							</div>
								...
								<label for="inputEmail" class="control-label"></label>
									<select class="form-control select2" name="author_id">
											...
											@foreach ($users as $user)
												<option value="{{$user->id}}" @if($blog->author_id == $user->id) selected @endif >{{$user->name}}</option>
											@endforeach
									</select>
							</div>
							</div>
							...
							<div class="form-group" >
								<label for="inputEmail" class="control-label"></label>
									...
									@if (isset($blog->image) && $blog->image!='')
										<img src="{{url('public/uploads/images/'.$blog->image)}}" style="width: 100PX;">
									@endif
							</div>
							</div>
							...
							<div class="col-md-3" style="margin-top:15px">
								...
								<select class="form-control" name="lang">
									@foreach($locales as $loc)
										<option value="{{ $loc }}" {{ ($blog->lang ?? 'en') === $loc ? 'selected' : '' }}>{{ strtoupper($loc) }}</option>
									@endforeach
								</select>
							</div>
							<div class="col-md-5" style="margin-top:15px">
								...
								<select class="form-control select2" name="translation_of">
									...
									@foreach($translation_parents as $p)
										<option value="{{ $p->id }}" {{ ($blog->translation_of ?? null) == $p->id ? 'selected' : '' }}>[{{ strtoupper($p->lang) }}] {{ $p->title }}</option>
									@endforeach
								</select>
							</div>
							...
							<div class="col-md-6" style="margin-top:15px">
								...
								<input type="text" name="meta_title" class="form-control" maxlength="70" value="{{ $blog->meta_title ?? '' }}">
							</div>
							<div class="col-md-6" style="margin-top:15px">
								<label class="control-label">Meta Description</label>
								<input type="text" name="meta_description" class="form-control" maxlength="160" value="{{ $blog->meta_description ?? '' }}">
							</div>
