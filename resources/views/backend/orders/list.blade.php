                   ...
                   <table class="table table-full table-full-small">
                      ...
                      <tbody id="myTable">
                        ...
                        <tr>
                            ...
                            <td>{{$order->package_number_of_question}}</td>
                            <td><span style="color:green">{{$order->answ}}</span></td>
                            <td><span style="color:red">{{$order->unansw}}</span></td>
                            <td>{{number_format($order->package_amount, 2)}}EUR</td>
                            <td><a href="{{url('/dashboard/orders/'.$order->id)}}">Reply</a>
                            	<a   onclick="delete_record('{{$order->id}}');"><i class="md md-delete red darken-2 icon-color icon-color-custom"></i></a>
                            </td>
                        </tr>
                        @endforeach
					  					</tbody>
...
function delete_it(){
	if(single_delete==='1'){
		var final_url=APP_URL+'/dashboard/orders/delete/'+del_id;
